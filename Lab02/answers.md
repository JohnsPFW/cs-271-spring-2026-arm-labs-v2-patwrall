...
- In `ADD`, op=0 and S=0. In `SUBS`, op=1 (SUB operation) and S=1. The S bit (bit 29) being set to 1 signals that condition flags should be updated. The op bit (bit 30) being set to 1 changes the operation from add to subtract.
...
- `MOVZ X5, #0xffff`: X5 = 0x000000000000ffff (writes bits 15-0, zeros the rest)
- `MOVK X5, #0x00ff, LSL #16`: overwrites bits 31-16 with 0x00ff, keeps bits 15-0
...
- What is the base instruction that `CMP X0, X1` translates to? `SUBS`
